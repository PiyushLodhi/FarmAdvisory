...
def main():
	...
	# f = open('unstructured_text_book_input_maize.txt')
	# lines = f.readlines()
	# input_lines = []
	# for line in lines:
	...
	# 		for slot in slots:
	# 			if slot['entity']!='crop':
	# 				if relation not in _dict['maize'].keys():
	# 					# print('case1: ')
	# 					# print('relation: '+str(relation))
	# 					_dict['maize'][relation] = slot['rawValue']
	# 				else:
	# 					# print('case2: ')
	# 					# print('relation: '+str(relation))
	# 					_dict['maize'][relation] += (','+slot['rawValue']) 
	...
	# in_file.close()
	# with open("output_kg_maize.json", "w") as outfile:
	# 	json.dump(_dict, outfile)
	# with open('output_kg_maize.json', 'r') as openfile:
	# 	json_object = json.load(openfile)
	# print(json_object) 
	#reading and including structured data on disease management in knowledge graph
	# f = open('structured_maize_disease.txt','r')
	# lines = f.readlines()
	# f.close()
	# diseases_list = []
	# flag=0
	...
	# 	json_object = json.load(openfile)
	# print(json_object) 
	# f = open('structured_maize_post_prod.txt','r')
	# lines = f.readlines()
	# f.close()
	# line = lines[0][:-1]
