...
def main():
	...
	# f = open('unstructured_text_book_input_wheat.txt')
	# lines = f.readlines()
	# input_lines = []
	# for line in lines:
	...
	# 			in_line = []
	# 			in_line.append('Wheat')
	# 		else:
	# 			in_line.append(word)
	# 	if in_line!=[]:
	# 		input_lines.append(in_line)
	# f.close()
	# f = open('input_wheat.txt','w+')
	...
	# engine.fit(dataset)
	# # with open('output_kg_rice.json', 'r') as openfile:
	# # 	_dict = json.load(openfile)
	# _dict = {}
	# _dict['wheat']={}
	# in_file = open('input_wheat.txt', 'r')
	...
	# 		for slot in slots:
	# 			if slot['entity']!='crop':
	# 				if relation not in _dict['wheat'].keys():
	# 					# print('case1: ')
	# 					# print('relation: '+str(relation))
	# 					_dict['wheat'][relation] = slot['rawValue']
	# 				else:
	# 					# print('case2: ')
	# 					# print('relation: '+str(relation))
	# 					_dict['wheat'][relation] += (','+slot['rawValue']) 
	...
	# in_file.close()
	# with open("output_kg_wheat.json", "w") as outfile:
	# 	json.dump(_dict, outfile)
	# with open('output_kg_wheat.json', 'r') as openfile:
	# 	json_object = json.load(openfile)
	# print(json_object) 
	#reading and including structured data on disease management in knowledge graph
	# f = open('structured_wheat_disease.txt','r')
	# lines = f.readlines()
	# f.close()
	# diseases_list = []
	# flag=0
	...
	with open('output_kg_wheat.json', 'r') as openfile:
		json_object = json.load(openfile)
	print(json_object) 
	f = open('structured_wheat_post_prod.txt','r')
	lines = f.readlines()
	f.close()
	line = lines[0][:-1]
