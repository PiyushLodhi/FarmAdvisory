...
def main():
	...
	f = open('unstructured_text_book_input_groundnut.txt')
	lines = f.readlines()
	input_lines = []
	for line in lines:
	...
	for line in lines:
		...
		if relation!='None' and prob>=0.3:
			...
			for slot in slots:
				if slot['entity']!='crop':
					if relation not in _dict['groundnut'].keys():
						# print('case1: ')
						# print('relation: '+str(relation))
						_dict['groundnut'][relation] = slot['rawValue']
					else:
						# print('case2: ')
						# print('relation: '+str(relation))
						_dict['groundnut'][relation] += (','+slot['rawValue']) 
	...
	in_file.close()
	with open("output_kg_groundnut.json", "w") as outfile:
		json.dump(_dict, outfile)
	with open('output_kg_groundnut.json', 'r') as openfile:
		json_object = json.load(openfile)
	print(json_object) 
	#reading and including structured data on disease management in knowledge graph
	# f = open('structured_groundnut_disease.txt','r')
	# lines = f.readlines()
	# f.close()
	# diseases_list = []
	# flag=0
	...
	# 	json_object = json.load(openfile)
	# print(json_object) 
	# f = open('structured_groundnut_post_prod.txt','r')
	# lines = f.readlines()
	# f.close()
	# line = lines[0][:-1]
